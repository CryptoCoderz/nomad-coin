 ...
 * The Nomad Developers 2011-2012
 */
...
NomadGUI::NomadGUI(QWidget *parent):
    QMainWindow(parent),
    clientModel(0),
    walletModel(0),
    toolbar(0),
...
{
    resize(850+95, 550);
    setWindowTitle(tr("Nomad") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/nomad"));
    setWindowIcon(QIcon(":icons/nomad"));
#else
...
void NomadGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a Nomad address"));
    ...
    aboutAction = new QAction(tr("&About Nomad"), this);
    aboutAction->setToolTip(tr("Show information about Nomad"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for Nomad"));
    optionsAction->setMenuRole(QAction::PreferencesRole);
    toggleHideAction = new QAction(QIcon(":/icons/nomad"), tr("&Show / Hide"), this);
...
void NomadGUI::setClientModel(ClientModel *clientModel)
{
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/nomad_testnet"));
            setWindowIcon(QIcon(":icons/nomad_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/nomad_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Nomad client") + QString(" ") + tr("[testnet]"));
...
void NomadGUI::setWalletModel(WalletModel *walletModel)
{
...
void NomadGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("Nomad client"));
...
#ifndef Q_OS_MAC
void NomadGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void NomadGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Nomad network", "", count));
}

void NomadGUI::setNumBlocks(int count)
{
...
void NomadGUI::message(const QString &title, const QString &message, bool modal, unsigned int style)
{
    QString strTitle = tr("Nomad") + " - ";
...
void NomadGUI::changeEvent(QEvent *e)
{
...
void NomadGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void NomadGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;
    ...
    QString strMessage = tr("This transaction is over the size limit. You can still send it for a fee of %1, "
        ...
        "Do you want to pay the fee?").arg(NomadUnits::formatWithUnit(clientModel->getOptionsModel()->getDisplayUnit(), nFeeRequired));
...
void NomadGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
        ...
        notificator->notify(Notificator::Information,
                            (amount)<0 ? tr("Sent transaction") :
                              ...
                              tr("Date: %1\n"
                                 ...
                                 "Address: %4\n")
                              .arg(date)
                              .arg(NomadUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(), amount, true))
                              .arg(type)
                              .arg(address), icon);
    }
}

void NomadGUI::gotoOverviewPage()
{
...
void NomadGUI::gotoAddressBookPage()
{
...
void NomadGUI::gotoBlockBrowser()
{
...
void NomadGUI::gotoReceiveCoinsPage()
{
...
void NomadGUI::gotoSendCoinsPage()
{
...
void NomadGUI::gotoSignMessageTab(QString addr)
{
...
void NomadGUI::gotoVerifyMessageTab(QString addr)
{
...
void NomadGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void NomadGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Nomad address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void NomadGUI::handleURI(QString strURI)
{
    // URI has to be valid
...
void NomadGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void NomadGUI::changePassphrase()
{
...
void NomadGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void NomadGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void NomadGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void NomadGUI::updateStakingIcon()
{
    updateWeight();
